...
(function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    // если количество стало 0 — удалить карточку
    if (qty === 0) {
      ...
      // если корзина теперь пустая — сообщение
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.location.pathname.startsWith("/carts")) {
      loadCarts().then(() => {
        ...
        updateCartTotal(); // <--- пересчёт суммы после загрузки карточек
      });
    } else {
      updateAllFavoriteButtons();
      updateAllCartButtons();
  ...
  async function loadCarts() {
    ...
    try {
      ...
      // вставляем HTML карточек в контейнер
      container.innerHTML = html;

      // после вставки — обновим вид кнопок и пересчитаем итоговую сумму
      updateAllFavoriteButtons();
      updateAllCartButtons();
      updateCartTotal();
    } catch (err) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.location.pathname.startsWith("/carts")) {
      loadCarts(); // <-- теперь updateCartTotal вызывается внутри loadCarts
    }

    // На остальных страницах — обновим визуал кнопок
    if (!window.location.pathname.startsWith("/favorites") &&
        !window.location.pathname.startsWith("/carts")) {
      updateAllFavoriteButtons();
      updateAllCartButtons();
    }
  });
